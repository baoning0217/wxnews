...
public interface MessageDao {
    ...
    @Select({" select ", SELECT_FIELDS, ", count(id) from ( select * from ", TABLE_NAME, " where from_id=#{userId} or to_id=#{userId} order by id desc ) tt group by conversation_id order by id desc limit #{offset}, #{limit}"})
